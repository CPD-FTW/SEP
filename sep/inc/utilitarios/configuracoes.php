...
if($_POST["btBoleto"] == "Boleto"){
	...
	$sql_configuracoes = mysql_query("SELECT outras_informacoes,codigo, endereco, cidade, estado, cnpj, email, secretaria, secretario, chefetributos, lei, decreto, topo_nfe, logo_nfe, brasao_nfe, codlayout, taxacorrecao, taxamulta, taxajuros, data_tributacao, declaracoes_atrazadas, gerar_guia_site FROM configuracoes");
	list($outras_informacoes, $codigo,$endereco,$cidade,$estado,$cnpj,$email,$secretaria,$secretario,$chefetributos,$lei,$decreto,$topo,$logo,$brasao,$layout,$taxacorrecao,$taxamulta,$taxajuros,$data_tributacao,$declaracoes_atrazadas,$gerar_guia_site) = mysql_fetch_array($sql_configuracoes);
?>
<head>
<script type="text/javascript">
function blocTexto(id)
{
    quant = 250;
    total = id.value.length;
    if(total <= quant)
    {
        resto = quant - total;
    ...
    {
        id.value = id.value.substr(0,quant);
    }
}
</script></head>
		...
		<form method="post" id="frmConfiguracoes" enctype="multipart/form-data">
			...
			<fieldset><legend>Configurações</legend>
				<table width="100%">
					...
					-->
                    <tr>
                    	<td colspan="4" align="left"><strong>Outras Informações: </strong></td>
                    </tr>
                    <tr>
                    	<td colspan="4">
                        
                        <textarea style="width:100%; height:100px;" onkeyup="blocTexto(this)" id="outras_informacoes" name="outras_informacoes"><?php print $outras_informacoes; ?></textarea><br />
                        <div style="float:right">caracteres restantes: <span id="cont">250</span></div>
                        </td>
                    </tr>
					<tr>
					...
					<tr>
						<td align="left" colspan="4">
							<?php
							if($brasao){
								//http://pmbp.rj.gov.br/efiscal/enota_new/img/brasoes/prefeitura.png
								//http://pmbp.rj.gov.br/efiscal/enota_new/sep/img/brasoes/prefeitura.png
							?>
