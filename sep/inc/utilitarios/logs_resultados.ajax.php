...
if($hidden == 'P'){	
	...
	if(!empty($nome) && !empty($data)){
		$string = "(cadastro.nome LIKE '%nome%' OR cadastro.razaosocial LIKE '%$nome%') AND SUBSTRING(logs_cadastro.data,1,10) = '$data' AND";
	}elseif(!empty($nome) && empty($data)){
		$string = "cadastro.nome LIKE '%nome%' OR cadastro.razaosocial LIKE '%$nome%' AND";
	}elseif(empty($nome) && !empty($data)){
		$string = "SUBSTRING(logs_cadastro.data,1,10) = '$data' AND";
	}else{
		$string = "";
	}
	...
	$query = ("
		SELECT
			IF(cadastro.razaosocial <> '', cadastro.razaosocial, cadastro.nome) AS usuario,
			logs_cadastro.ip,
			logs_cadastro.data,
			logs_cadastro.acao
		FROM
			logs_cadastro
		INNER JOIN
			cadastro ON cadastro.codigo = logs_cadastro.codcadastro
		WHERE
			$string cadastro.codtipo = '$codtipo'
		ORDER BY 
			logs_cadastro.codigo DESC				
	");
}
?>
<fieldset>
