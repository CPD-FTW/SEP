...
class ArquivoRetorno {
	...
	function registrarPagamentoManual($nossonumero, $valor){
		$numero1=substr($nossonumero,20,3);
		$numero2=substr($nossonumero,24,11);
		$numero3=substr($nossonumero,36,11);
		$nn=$numero1.$numero2.$numero3;
		$valor = MoedaToDec($valor);
		$sql = mysql_query("SELECT codigo FROM guia_pagamento WHERE nossonumero = '$nn' AND valor = $valor");
		if (mysql_num_rows($sql) == 0) {
			return false;
		} else {
			list($cod_guia) = mysql_fetch_array($sql);
