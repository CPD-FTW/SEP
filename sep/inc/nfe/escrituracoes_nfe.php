...
if ($_POST['btConfirmar']) {
	if ($_POST['txtNossoNumero'] || $_POST['txtValorBoleto']) {
		$contcodbarras = strlen($_POST['txtNossoNumero']);
		if($contcodbarras==48){
			$notas = ArquivoRetorno::registrarPagamentoManual($_POST['txtNossoNumero'],$_POST['txtValorBoleto']);
			if ($notas) {
				Mensagem_onload("{$notas} notas escrituradas");
			} else {
				Mensagem_onload("Verifique o Nosso N&uacute;mero e o Valor");
			}
		}else{
			Mensagem_onload("C&oacute;digo de barra inv&aacute;lido, o mesmo deve ter 48 n&uacute;meros.");
		}
	} else {
		...
		<td align="left">
			...
			<form method="post" enctype="multipart/form-data">
				...
				<fieldset><legend>Pagamento Manual</legend>
					<table width="100%">
						<tr>
							<td width="120">Código de Barras<font color="#FF0000">*</font></td>
							<td><input class="texto" size="55" name="txtNossoNumero" onblur="maskIt(this,event,'################################################')" onchange="maskIt(this,event,'################################################')" onkeyup="maskIt(this,event,'################################################')" onkeydown="maskIt(this,event,'################################################')" onkeypress="maskIt(this,event,'################################################')" id="txtNossoNumero" type="text" /></td>
						</tr>
						<tr>	
