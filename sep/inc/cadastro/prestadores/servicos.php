  ...
  <tr id="camposservico<?php echo $contservico; ?>" style="display:<?php echo $display; ?>">
    <td align="left" bgcolor="#999999">
     ...
     <option value="">Categoria</option>
	  <?php
	  $sql_categoria=mysql_query("SELECT codigo,nome FROM servicos_categorias ORDER BY codigo ASC");
	  while(list($codcat,$nomecat)=mysql_fetch_array($sql_categoria))
	  {
	    print("<option value=\"$codcat|$contservico|$maxcodcat\">$codcat - $nomecat</option>");
	  }
	  ?>
	 ...
	 {?>
		 ...
		 <select name="cmbCodigo<?php echo $codcategoria.$contservico; ?>" id="cmbCodigo<?php echo $codcategoria.$contservico; ?>" style="width:440px">
		   ...
		   <option value="">Código | Descrição</option>
		   <?php
		   // laco para display das opcoes no combo
		   while(list($codigo, $codservico,$codcategoriaConcatena,$descricao, $aliquota, $estado) = mysql_fetch_array($sql_servicos)) {
				//print("<option value=$codigo>$codservico | ".substr($descricao,0,70)."... | $aliquota</option>");
				//print("<option value=$codigo>$codservico | ".substr($descricao,0,70)."... </option>");
				$codservico = str_replace_once($codcategoriaConcatena,$codcategoriaConcatena.".",$codservico);
				print("<option value=$codigo>$codservico | ".substr($descricao,0,70)."... </option>");
		   } // fecha while
		   ?>
		 </select><?php echo $obrigatorio; ?>
