...
if($_POST['btCancelar']){
	$codguia = $_POST['hdCodguia'];//verifica se não vem valor vazio.
	if ($codguia != ''){
		$sql = mysql_query("SELECT codlivro FROM guia_pagamento WHERE codigo = '$codguia'");
		list($codlivro) = mysql_fetch_array($sql);
		mysql_query("UPDATE guia_pagamento SET estado = 'C' WHERE codigo = '$codguia'");
		mysql_query("UPDATE livro SET estado = 'C' WHERE codigo = '$codlivro'");
		
		echo "<script>alert('Guia cancelada com sucesso!')</script>";
	} else {
		 return false ;
	}
}
